...
template <class TBase>
 inline CValue CHtmEngineCtrlT<TBase>::GetDlgItemValue(HWND hwndControl)
 {
    ATLASSERT(::IsWindow(m_hWnd));
    ATLASSERT(::IsWindow(hwndControl));
    ATLASSERT(::GetParent(hwndControl) == m_hWnd);

    CString type = GetDlgItemAttribute(hwndControl,TEXT("type")).ToString();
...
WINDOW_TEXT_AS_VALUE:
    ...
    else if(eq(type,TEXT("select")))
    {
      ...
      CValue size = GetDlgItemAttribute(hwndControl,TEXT("size"));
    ...
    {
      // our HTML editor 
      CHtmEngineEditorCtrl htmed = hwndControl;
      CValue v;
...
template <class TBase>
 inline void CHtmEngineCtrlT<TBase>::GetDlgItemValues(CSimpleMap<CString,CValue>& bag)
 {
...
template <class TBase>
 inline void CHtmEngineCtrlT<TBase>::SetDlgItemValue(HWND hwndControl, CValue v)
{
    ATLASSERT(::IsWindow(m_hWnd));
    ATLASSERT(::IsWindow(hwndControl));
    ATLASSERT(::GetParent(hwndControl) == m_hWnd);
...
template <class TBase>
  inline void CHtmEngineCtrlT<TBase>::SetDlgItemValue(LPCTSTR name, CValue v)
{
...
template <class TBase>
 inline void CHtmEngineCtrlT<TBase>::SetDlgItemValues(const CSimpleMap<CString,CValue>& bag)
 {
